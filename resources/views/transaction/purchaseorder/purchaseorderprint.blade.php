...
<table style="margin-bottom: 20px;">
  <tr>
    <td rowspan="5" style="width:100px; padding-right:10px;">
      <img src="/storage/company/{{ $purchaseOrder->location->company->logo_filename }}" alt="Logo" style="width:100%;">
    </td>
  </tr>
  <tr>
    <td><h2 style="margin-bottom: 0px;">{{ $purchaseOrder->location->company->company_name }}</h2></td>
  </tr>
  <tr>
    <td>Address: {{ $purchaseOrder->location->company->address }}</td>
  </tr>
  <tr>
      <td>Contact No.: {{ $purchaseOrder->location->company->contact_no }}</td>
  </tr> 
  <tr>
    <td>Website: {{ $purchaseOrder->location->company->website }} Email: {{ $purchaseOrder->location->company->email }}</td>
  </tr> 
</table>
@endsection
...
<table class="tbl header" id="table" style="margin-top: 20px;" cellspacing="0">
  <tbody>
      ...
      <tr>
        <td rowspan="2" class="bg-dark">Address:</td><td rowspan="2"><span class="data">{{ $purchaseOrder->supplier->address }}</span></td>
        <td class="bg-dark">Purchasing Date:</td><td><span class="data">{{ $purchaseOrder->purchase_date }}</span></td>
      </tr>
      <tr>
        <td class="bg-dark">Terms:</td><td><span class="data">{{ $purchaseOrder->terms }}</span></td>
      </tr>
      {{-- <tr>
...
<div id="tableDataWrapper">
  <table class="tbl" style="margin-bottom: 0px;" id="tableData" cellspacing="0">
    <thead>
      <tr>
